...
| cheap tier (CIFAR-10 / ImageNet-64 / SiT latent) | **built** — FINDINGS §6; ~230–455 GPU-h instead of ~5,000 |
| λ ladder to 16,384 dims, inference only | **built** — `exp/10_lambda_real.py`; no training, ~10 GPU-h |
| GPU budget | RUNPLAN.md — the cheap programme on 4090s, ~130 GPU-h |
| GPU pre-flight | **done** — `scripts/smoke.sh --gpu` on an RTX 4090, all suites pass; `results/probe_4090_smoke.txt` |
| GPU runs | not started — RUNBOOK.md is the ordered command list, with GPU counts per step |
...
ddgpu/      the real codebase (DiT, EDM/VP/interpolant, teacher zoo, DMD2, eval)
exp/        experiments; 01 = lambda curve, 09 = run figures, 10 = the lambda ladder
...
RUNPLAN.md  GPU counts, memory math, wall-clock, sequencing (cheap tier)
RUNBOOK.md  the ordered commands, bare box -> Phase E, GPUs needed per step
```
...
# the cheap tier (FINDINGS §6) -- start here. ~230-455 GPU-h, not ~5,000.
python3 exp/10_lambda_real.py --teacher diffusers:google/ddpm-cifar10-32 \
    --data data/cifar10 --tag cifar10_3072      # Figure 1, inference only
...
scripts/eval_all.sh -d cifar10 --gpus 0 --n 50000

# the full ImageNet-256/512 programme, once the cheap tier justifies it
...
2. `RUNBOOK.md` — what to run, in order, and how many 4090s each step needs.
3. `RUNPLAN.md` — GPU counts, memory math, wall-clock, sequencing, and why the
   ImageNet-256/512 programme is deferred.
4. `LOG.log` — the chronological research narrative, newest entry last. Read this
   when you need to know *why* something is the way it is, including the things
   that were tried and abandoned.
5. `scripts/README.md` — how to actually run any of it on a GPU box, and the
   three preconditions the launcher enforces rather than documents.
6. `ddgpu/robust.py` and `ddgpu/invertible.py` — the two methods; the module
   docstrings carry the derivations and the known failure modes.
7. `ddgpu/vp.py` and `ddgpu/interpolant.py` — why released teachers need a change
   of variables before any of this applies to them, and what breaks silently if
   they do not get one.
8. `ddgpu/teachers.py` — the one door all four preconditioning families go
   through, and `validate_teacher`, which is what notices when one is wired
   wrong (no exception is raised otherwise).
